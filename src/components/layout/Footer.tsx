...
/**
 * Anzeigenamen der Plattformen, falls die Config kein eigenes `label` setzt.
 * Ohne diese Tabelle landete der Maschinenschlüssel im Vorlesetext, die
 * Icon-Links im Footer hießen für Screenreader "discord" und "github".
 */
            ...
            <div key={i}>
              ...
              <ul className="space-y-2 text-sm">
                {col.links.map((link, j) => {
                  ...
                  // Sprachabhängige Slugs auflösen. `/impressum` heißt auf der
                  // englischen Seite `/imprint`, `/datenschutz` heißt
                  // `/privacy`. Vorher wurde das Präfix nur vorangestellt, die
                  // englischen Rechtslinks liefen also auf die
                  // Nicht-gefunden-Seite. `alternatePath` ist dieselbe Quelle,
                  // aus der auch die hreflang-Alternates kommen.
                  const href =
                    link.external || isExternal(link.href)
                      ? link.href
                      : `/${locale}${alternatePath(link.href, locale)}`;
                  return (
                    <li key={j}>
                      <Link
                        href={href}
          ...
          <div className="flex items-center gap-3">
            {footerConfig.social.map((s, i) => (
              <a
                ...
                aria-label={s.label ?? PLATFORM_NAMES[s.platform] ?? s.platform}
